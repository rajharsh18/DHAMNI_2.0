...
try {
    ...
    $check = $_SESSION['name'];
    $sql = "SELECT name, reg_no FROM `blood_bank` WHERE `name` = '$check'";
    ...
    if (isset($_POST['reg_no'])) {
        $reg_no = $_POST['reg_no'];
        $name = $_POST['name'];
        $sql2 = "DELETE FROM `blood` WHERE '$reg_no' = `Blood_bank_id`";
        $sql1 = "DELETE FROM `blood_bank` WHERE '$reg_no' = `Reg_no` and '$name' = `name`";
    ...
    <div class="card">
        <form action="blood_bank_delete.php" class="box" method="post">
            ...
            <input type="text" name="reg_no" class="form-control" id="inputReg" placeholder="Registration Number"
                required>
            <input type="text" name="name" class="form-control" id="inputName" placeholder="Name of Blood Bank"
                required>
