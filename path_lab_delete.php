...
try {
    ...
    $check = $_SESSION['name'];
    $sql = "SELECT name, reg_no FROM `path_lab` WHERE `name` = '$check'";
    ...
    if (isset($_POST['reg_no'])) {
        $reg_no = $_POST['reg_no'];
        $name = $_POST['name'];
        $sql1 = "DELETE FROM `path_lab` WHERE '$reg_no' = `Reg_no` and '$name' = `name`";
    ...
    <div class="card">
        <form action="path_lab_delete.php" class="box" method="post">
            ...
            <input type="text" name="reg_no" class="form-control" id="inputReg" placeholder="Registration Number"
                required>
            <input type="text" name="name" class="form-control" id="inputName" placeholder="Name of Path Lab" required>
